...
export default async function Widgets() {
  ...
  const RandomCat = Math.floor(Math.random() * newsCategory.length);
  const newsResults = await fetch(
    `https://newsapi.org/v2/top-headlines?country=in&category=${newsCategory[RandomCat]}&sortBy=publishedAt&pageSize=5&apiKey=fa594d639ccb4aa4b3369c7e9b1f2d9c`
  ).then((res) => res.json());
  ...
  return (
    <div className="xl:w-[600px] hidden lg:inline ml-8 space-y-5">
      ...
      <div className="sticky top-16 text-gray-700 space-y-3 bg-gray-100 pt-2 rounded-xl w-[90%] xl:w-[75%]">
        ...
        {randomUsersResults?.results.map((randomUser) => {
          return (
            <div
